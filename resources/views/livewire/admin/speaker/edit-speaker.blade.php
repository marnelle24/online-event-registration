                    ...
                    <form wire:submit.prevent="save" class="flex flex-col gap-4">
                        <div>
                            <p class="italic text-md text-slate-500">
                                ...
                                Changes will be reflected across all programmes where this speaker is assigned.
                            </p>
                        </div>
                        ...
                        <div class="space-y-4 flex flex-col gap-4">
                            ...
                            <div>
                                ...
                                @if(is_array($socials))
                                    ...
                                    @endforeach
                                @else
                                    <p class="text-sm text-slate-500">No social media accounts configured.</p>
                                @endif
                                <button 
                                    type="button"
                            ...
                            <div class="flex flex-col gap-3 border border-slate-300 rounded-md p-2">
                                <!-- Current Profile Picture -->
                                @if($speaker->getFirstMediaUrl('speaker'))
                                    <div>
                                        <label class="mb-1 block text-sm font-medium text-black">Current Profile Picture</label>
                                        <img src="{{ $speaker->getFirstMediaUrl('speaker') }}" alt="{{ $speaker->name }}" class="w-20 h-20 rounded-full object-cover border border-slate-300">
                                    </div>
                                @endif

                                <!-- Thumbnail Upload -->
                                <div class="flex flex-col gap-2">
                                    <label class="mb-1 block text-sm font-medium text-black">{{ $speaker->getFirstMediaUrl('speaker') ? 'Update Profile Picture' : 'Profile Picture' }}</label>
                                    <input 
                                    ...
                                    @if($thumbnail)
                                        <div>
                                            <label class="block text-sm font-medium text-black">Profile Picture Preview</label>
                                            <img src="{{ $thumbnail->temporaryUrl() }}" alt="Profile picture preview" class="w-20 h-20 rounded-full object-cover border border-dashed border-slate-300">
                                        </div>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="flex justify-between items-center gap-4">
                                <button 
                                    ...
                                    type="submit" 
                                    class="disabled:cursor-not-allowed disabled:opacity-50 w-full flex justify-center p-4 items-center rounded-md text-md text-white drop-shadow uppercase bg-blue-700 hover:bg-blue-600 duration-300">
