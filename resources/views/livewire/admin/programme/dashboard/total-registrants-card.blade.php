<div class="rounded-sm border border-stroke bg-white px-7.5 py-6 shadow-default">
    <div class="flex h-11.5 w-11.5 items-center justify-center rounded-full bg-meta-2">
        <svg class="fill-primary" width="22" height="18" viewBox="0 0 22 18" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="mt-4 flex items-end justify-between">
        <div>
            <h4 class="text-title-md font-bold text-black">{{ number_format($totalRegistrants) }}</h4>
            <span class="text-sm font-medium">Total Registrants</span>
            <p class="text-xs text-slate-500 mt-1">Last 7 days: {{ number_format($recentRegistrants) }} new sign-ups</p>
        </div>

        <span class="flex items-center gap-1 text-sm font-medium {{ $growthPercentage !== null ? ($growthPercentage >= 0 ? 'text-emerald-600' : 'text-rose-600') : 'text-slate-400' }}">
