    ...
    <div class="flex justify-between gap-3 mb-8 lg:flex-row flex-col lg:items-center items-start">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 lg:gap-3 gap-1">
            <button wire:click="exportCsv"
                class="flex items-center gap-2 border border-slate-500 bg-slate-100 drop-shadow text-slate-500 hover:text-slate-500 hover:bg-slate-300 hover:border-slate-400 rounded-full hover:-translate-y-0.5 duration-300 py-2 px-5">
            ...
            <table class="w-full table-auto">
                <thead>
                    <tr class="bg-slate-200 border border-slate-300 text-slate-500 text-left">
                        ...
                        <th class="p-4 font-bold text-nowrap cursor-pointer hover:bg-slate-300" wire:click="sortByStatus">
                            Status
                            @if($sortBy === 'is_active')
                                <span class="ml-1">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                            @else
                    ...
                    @if($users->count())
                        @foreach ($users as $user)
                            <tr class="hover:bg-slate-400/10 duration-300 border border-slate-300 dark:border-strokedark bg-white">
                                ...
                                <td class="p-4">
                                    <div class="flex text-nowrap">
                                        @if($user->ministries->count() > 0)
                                            ...
                                            @endif
                                        @else
                                            <span class="text-xs text-slate-400 text-nowrap">No ministries</span>
                                        @endif
                                    </div>
                                </td>
                                <td class="p-4">
                                ...
                                <td class="">
                                    <div x-data="{ actionToolTip: false, optionBox: false }" class="relative flex justify-end items-center gap-1.5">
                                        <button x-cloak type="button" class="relative flex justify-center items-center gap-3 hover:scale-110 transition-all duration-300">
                                            <svg 
                                                @click="optionBox = true, showToolTip = false"
                                                @click.outside="optionBox = false, showToolTip = false"
                                                @mouseover="actionToolTip = true" 
                                            ...
                                            <div x-show="actionToolTip" x-transition class="absolute top-5 -left-8 transition-all duration-300 ease-in-out hover:opacity-100 hover:translate-y-0 bg-slate-800 text-white text-xs rounded px-2 py-1 shadow-lg z-50">
                                                Actions
                                            </div>
                                        </button>
                                        <div x-show="optionBox" x-transition class="absolute top-5 right-2 transition-all duration-300 ease-in-out hover:opacity-100 hover:translate-y-0 bg-white text-white shadow-lg z-50 min-w-max rounded-md">
                                            <ul class="">
                                                <li wire:click="callEditUserModal({{ $user->id }})" class="text-slate-700 hover:bg-slate-200 flex items-center gap-2 text-sm border-b border-slate-300 px-3 py-2 cursor-pointer">
                                                    <svg class="w-4 h-4 stroke-blue-500 hover:stroke-blue-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                                    </svg>
                                                    Edit Record
                                                </li>
                                                <li wire:click="deleteUser({{ $user->id }})" class="text-slate-700 hover:bg-slate-200 flex items-center gap-2 text-sm border-b border-slate-300 px-3 py-2 cursor-pointer">
                                                    <svg class="w-4 h-4 stroke-red-500 hover:stroke-red-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                                                    </svg>
                                                    Delete
                                                </li>
                                                <li wire:click="callResetPasswordModal({{ $user->id }})" class="text-slate-700 hover:bg-slate-200 flex items-center gap-2 text-sm border-b border-slate-300 px-3 py-2 cursor-pointer">
