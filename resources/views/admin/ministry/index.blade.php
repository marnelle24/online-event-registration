...
<x-app-layout>
    <div class="flex justify-between gap-3 mb-8">
        ...
        <a wire:navigate href="{{ route('admin.ministry.create') }}" 
        ...
        @foreach ($ministries as $ministry)
            <div class="flex flex-col border border-slate-400/70 rounded-lg bg-zinc-100 dark:bg-slate-700 hover:bg-neutral-200/70 hover:-translate-y-0.5 dark:hover:bg-slate-800/90 duration-300 shadow-md">
                <div class="flex gap-3 p-4 bg-zinc-200 border-b border-slate-400/70 dark:bg-slate-800 rounded-t-lg items-center">
                    <p class="w-8 h-8 border border-zinc-800/40 dark:border-zinc-100/40 text-md font-light rounded-full text-slate-700 dark:text-slate-100 text-center flex items-center justify-center bg-slate-100 dark:bg-slate-400">
                        {{ Helper::getInitials($ministry->name) }}
                    </p>
                    <h4 class="text-xl text-slate-700 dark:text-slate-100 capitalize leading-none">{{ $ministry->name }}</h4>
                </div>
                <div class="p-6">
                    <table>
                        <tr>
                            <td class="text-sm py-1.5 text-center text-slate-600 dark:text-white">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                                </svg>
                            </td>
                            <td class="text-sm p-1.5 text-slate-600 dark:text-white capitalize font-light"> {{ $ministry->contactPerson ? $ministry->contactPerson : 'N/A' }}</td>
                        </tr>
                        <tr>
                            <td class="text-sm py-1.5 text-center text-slate-600 dark:text-white">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                            ...
                            <td class="text-sm p-1.5 text-slate-600 dark:text-white"> {{ $ministry->contactNumber ? $ministry->contactNumber : 'N/A' }}</td>
                        </tr>
                        <tr>
                            <td class="text-sm py-1.5 text-center text-slate-600 dark:text-white">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                            ...
                            <td class="text-sm p-1.5 text-slate-600 dark:text-white"> {{ $ministry->contactEmail ? $ministry->contactEmail : 'N/A' }}</td>
                        </tr>
                        <tr>
                            <td class="text-sm py-1.5 text-center text-slate-600 dark:text-white">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                            ...
                            <td class="text-sm p-1.5 text-slate-600 dark:text-white"> 
